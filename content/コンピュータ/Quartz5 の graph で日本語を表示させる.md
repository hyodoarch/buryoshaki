...
> [!info]
> 2026-09-07
> 本家 graph + deploy.yml でうまくいったと思ったのですが、ノードのマウスオーバーが不安定で、モバイルはグラフビュー表示自体が不安定なので、graph-ja に戻しました。



> [!info]
> 2026-09-06
> フォークしなくても、deploy.yml へコードを追記することにより、日本語表示が可能なことが分かりました。この場合、下記のような流れになります。
> 
> Graph内のutilsを0.1.1へ一時的に差し替え
> ↓
> Graphを再ビルド
> ↓
> Quartzサイト全体をビルド
...
> GitHub Pagesへデプロイ
> 
> すでに Graph の日本語問題はすでに本家で原因まで特定されており、その修正も `@quartz-community/utils 0.1.1` として公開済みでした。`utils 0.1.1` では、URL の非ASCII文字を正しくデコードする修正が入っています。そのうち次期マイナーバージョンアップで修正されると思われます。
